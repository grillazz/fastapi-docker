...
async def worker_task():
    ...
    logging.info(
        f"Worker running at {current_time} | PID: {str(os.getpid())} {str(socket.gethostname())}"
    )
    await anyio.sleep(2.0)
    _trip = httpx.get(
        f"http://dispatch-service:8000/api/v1/trips/{str(socket.gethostname())}"
    ).json()
    if not _trip:
    ...
    await anyio.sleep(2.0)
    httpx.patch(
        f"http://dispatch-service:8000/api/v1/trips/{_trip['trip_id']}",
